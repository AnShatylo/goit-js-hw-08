...
function getImage(evt) {
  ...
  const child = evt.target.closest(".gallery-image");
  const currentOriginalImg = child.dataset.source;
  const currentImg = images.find(
    ({ original }) => original === currentOriginalImg
  );
...
gallery.addEventListener("click", getImage);
